...
class Ifsoac:
    def __init__(self, series=None, **op):
        ...
        series = [np.random.rand(op.nrandom)] if series is None else series
        if len(series) > 100:
            series = [series]
        poligono = p_regular(op.nsides, op.rotate)
        scaler = preprocessing.MinMaxScaler(feature_range=(0, op.nsides))
        self.ifs = []
        for serie in series:
    ...
    def jDC(self, serie=None):
        ...
        if serie is None:
            serie = self.ifs[0]
        _jdc = jdc(serie, op.p)
        return _jdc if op.transform is None else op.transform(_jdc, op.argv)
    ...
    def _images_ds(self):
        ...
        for serie in self.ifs:
            if op.x_range:
                cv = ds.Canvas(plot_width=vent, plot_height=vent, x_range=op.x_range, 
                               y_range=op.y_range)
            else:
                cv = ds.Canvas(plot_width=vent, plot_height=vent)
            df = pd.DataFrame(self.jDC(serie), columns=cols)
            if op.transform == escher:
                df = pd.concat([df, pd.DataFrame(p_regular(10000), columns=cols)])
            agg = cv.points(df, *cols)
            if isinstance(op.cmap_ds, str):
                r = tf.shade(agg, cmap=palette[op.cmap_ds])
            else:
                r = tf.shade(agg, cmap=op.cmap_ds)
            if op.fatpoints:
                r = tf.spread(r)
            if op.background_ds is not None:
                r = tf.set_background(r, op.background_ds)
            res.append(r)
        return res
    ...
    def to_dataframes(self):
        return [pd.DataFrame(self.jDC(serie), columns=list('xy')) for serie in self.ifs]
